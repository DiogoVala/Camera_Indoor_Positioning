...
frame_height=960
RESOLUTION = (1280, 960)
picamera.PiCamera().resolution = (frame_width, frame_height)
frame_cap = np.empty((RESOLUTION[1], RESOLUTION[0], 3), dtype=np.uint8)
...
while True:
	...
	picamera.PiCamera().capture(frame_cap, 'rgb')
	
	frame = frame_cap.copy()
